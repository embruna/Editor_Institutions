...
AnalysisData <- AnalysisData %>% filter(YEAR>0|is.na(YEAR)==FALSE)
##############################################################
...
editorAcum<-AnalysisData %>% group_by(YEAR,INST) %>% summarize(yr_tot = n_distinct(INST))
editorAcum<-spread(editorAcum, INST,yr_tot) 
...
# Used in Figure 1A
GEOperYR<-AnalysisData %>% group_by(YEAR) %>% summarize(AnnualRichness = n_distinct(INST)) 
GEOperYR
##############################################################
...
plotPOOLEDsimpdiv<-ggplot(IsimpDivTable, aes(x=YEAR, y=InvSimpson)) +
  geom_point(size=1, color="blue") + # Use hollow circles
  xlab("Year")+
  ylab(bquote('D'[2]))+
