...
# This takes the final data from 2014, and uses that to collect data on
# subsequent years by recording who was present from the 2014 list in a 
# future year, who was no longer on the list, and adding names that needed to be 
# added.
AreiasData <- function(original_data) {
...
# new_fem<-read_csv("./data_raw/areias_data/fem_2016-2024.csv") %>% 
#   mutate(journal="fem") %>% 
#   mutate(across(where(is.character), ~na_if(., "--"))) %>% 
#   mutate(across(starts_with("yr_"), ~replace_na(.,"N")))

  
# This function will take the data for each journal (2014 editors + columns 
# indicating if they are on the editor list in subsequent years). These data 
# then get converted from wide to long form in preparation for adding to 
# The data from previous years.
  
  new_data_cleaner <- function(new_data) {
     new_data<-new_data %>% 
      mutate(across(where(is.character), ~na_if(., "--"))) %>% 
       mutate_all(tolower) %>% 
      # mutate(across(starts_with("yr_"), ~replace_na(.,"N"))) %>% 
  ...
  # JAPE
  jape<-read_csv("./data_raw/areias_data/jape_2015-2018.csv") %>% 
    mutate(journal="jape")
  jape<-new_data_cleaner(jape)
  
  # CONBIO
  conbio<-read_csv("./data_raw/areias_data/conbio_2014-2018.csv") %>% 
    mutate(journal="conbio")
  conbio<-new_data_cleaner(conbio)
  
  
  # ECOL
  ecol<-read_csv("./data_raw/areias_data/ecol_2014-2021.csv") %>% 
    mutate(journal="ecology")
  ecol<-new_data_cleaner(ecol)
  
  # newphyt
  newphyt<-read_csv("./data_raw/areias_data/newphyt_2015-2018.csv") %>% 
    mutate(journal="newphyt")
  newphyt<-new_data_cleaner(newphyt)
  
  # bitr
  bitr<-read_csv("./data_raw/areias_data/bitr_2014-2018.csv",locale = locale(encoding = "latin1")) %>% 
    mutate(journal="bitr") 
  # bitr$inst<-iconv(bitr$inst, from = 'UTF-8', to = 'ASCII//TRANSLIT',sub = "?")
  bitr<-new_data_cleaner(bitr)
  
  # plantecol
  plantecol<-read_csv("./data_raw/areias_data/plantecol_2015-2017.csv") %>% 
    mutate(journal="plantecol")
  plantecol<-new_data_cleaner(plantecol)
  
  # jecol
  jecol<-read_csv("./data_raw/areias_data/jecol_2014-2018.csv") %>% 
    mutate(journal="jecol")
  jecol<-new_data_cleaner(jecol)
  
  
# fem
fem<-read_csv("./data_raw/areias_data/fem_2016-2024.csv") %>% 
  mutate(journal="fem")
fem<-new_data_cleaner(fem)

# biocon
biocon<-read_csv("./data_raw/areias_data/biocon_2014-2024.csv") %>% 
  mutate(journal="biocon")
biocon<-new_data_cleaner(biocon)


# ajb
ajb<-read_csv("./data_raw/areias_data/ajb_2014-2018.csv") %>% 
  mutate(journal="ajb")
ajb<-new_data_cleaner(ajb)


# This is all the new data bound together 
new_data<-bind_rows(fem,
                    conbio,
                    ecol,
                    biocon,
                    jecol,
...
# Column names of the newly recorded data 
names_new_data<-names(new_data)

# Column names of the newly recorded data 
names_all<-names(original_data)
# The columns tthey DONT have in common
setdiff(names_new_data, names_all)
# The columns they DO Have in Common
intersect(names_new_data, names_all)

# reduce the new data to the columns they have in common
new_data<-new_data %>% 
  select(intersect(names_new_data, names_all))
names(original_data)

# Make sure you only have one per year
# (though note you can still have multiple editor roles)
match_new<-new_data %>% 
  select(journal,
         first_name,
         middle_name,
...
# Get the 2014 data for these journals

match_data<-original_data %>% 
  filter(journal%in% unique(new_data$journal)) %>% 
  filter(year=="2014") %>% 
  select(journal, 
         title, 
         first_name,
         ...
         country,
         editor_id) 
# do any have multiple rows (roles)? 
match_data %>% 
  group_by(last_name,
           middle_name,
           first_name,
...
# Left join original data into the new data. This will 
# give you the eidotr_ids for the existing ones 
# (those needing will also be identified)
new_editor_info<-left_join(match_new,
                     match_data,
                     by=c("journal",
                          "first_name",
                          "middle_name",
...
# names(new_editor_info)
intersect(names(original_data), names(new_editor_info))

# add the new data to the original dataset
original_data<-bind_rows(original_data,new_editor_info)
names(original_data)

# Are there any with duplicate roles?
original_data %>% 
  group_by(last_name,
           middle_name,
           first_name,
  ...
  filter(n>1)
####################### pick up here
# TODO: need to give editor_ids to the new editors added

need_ids<-original_data %>% 
...
new_data<-new_data %>% mutate(year=as.numeric(year))
  
new_data<-left_join(new_data,new_editor_info)

summary_original_data<-original_data %>% 
  group_by(journal,year) %>% 
  summarize(n_distinct(editor_id))

summary_original_data %>% filter(journal=="bitr") %>% arrange(desc(year))

original_data<-bind_rows(original_data,new_data)
...
summary_original_data<-original_data %>% 
  ...
  summarize(n_distinct(editor_id))
summary_original_data %>% filter(journal=="bitr") %>% arrange(desc(year))
multiples<-match_data %>% group_by(last_name,first_name) %>% 
  tally() %>% 
  filter(n>1)
names(original_data)
  ...
  inst_fix$year<-as.numeric(inst_fix$year)
